  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers) => {
      try {
        ...
        formData.append('archived', values.archive);
        if (values.picture) formData.append('Picture', values.picture);


        await axios.patch('http://localhost:8001/medicine/editMedicine', formData, {
          headers: { // done new Route
        ...
        //   if (values.archive) {
        //     // Archive medicine logic
        //     await axios.post('http://localhost:8001/medicine/archiveMedicine', { name: values.name });
        // } else {
        //     // Unarchive medicine logic
        //     await axios.post('http://localhost:8001/medicine/unarchiveMedicine', { name: values.name });
        // }
      } catch (err) {
        console.log(err)
                ...
                <Stack spacing={3} >
                  ...
                  <Stack spacing={3}>
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          defaultChecked={formik.values.archive}
                          onChange={() => formik.values.archive = !formik.values.archive}
                        />
                      }
